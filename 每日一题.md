...
给定一个数组 candidates（有重复元素，这是和上一题的区别所在） 和一个目标数 target ，找出 candidates 中所有可以使数字和为 target 的组合。
...
class Solution {
    ...
    private void backTrack(int[] candidates, int index, int target, LinkedList<Integer> list) {
        ...
        // 在 [index, length]之间选择一个下标i, [index, i]的部分不选
        for (int i = index; i < candidates.length; i++) {
            // 剪枝操作:遇到相同的数字交给下一次递归,避免出现相同的集合
...
class Solution {
    ...
    private void backTrack(int target, int i, int k, LinkedList<Integer> list) {
        if (k == 0) {
            if (target == 0) res.add(new LinkedList<>(list));
            return;
        }
        if (i >= 10) return;
        backTrack(target, i + 1, k, list);

        list.add(i);
        backTrack(target - i, i + 1, k - 1, list);
        list.removeLast();
    }
}
```
...
class Solution {
    ...
    private boolean dfs(char[][] board, String word, int i, int j, int index) {
        ...
        if (i < 0 || i >= board.length || j < 0 || j >= board[0].length || word.charAt(index) != board[i][j] || visited.contains(i * board[0].length + j)) return false;
